...
u8 Item_Give(PlayState* play, u8 item) {
    ...
    if ((item >= ITEM_MEDALLION_FOREST) && (item <= ITEM_MEDALLION_LIGHT)) {
        ...
        // No slot could hold the contents. Vanilla has no return here and falls through to the
        // default write at the end of the function, storing the contents in the first bottle
        // slot; this early return diverges from that to avoid clobbering a filled bottle.
        if (GameInteractor_Should(VB_BOTTLE_CONTENTS_REQUIRE_EMPTY_BOTTLE, true, item)) {
            return Return_Item(item, MOD_NONE, ITEM_NONE);
        }
