...
#define HEAPSIM_ACTOROVL_ABSOLUTE_SPACE_SIZE 0x27A0

// N64 play state heap size (see GameState_Realloc in Play_Init, which doubles
// it for SoH), minus init allocations N64 makes but SoH skips entirely:
// kaleido overlay area (0x26960), do-action texture buffer and minimap segment.
// Allocations SoH still makes are deducted live by GameState_Alloc hook.
static const size_t kHeapSimSize = 0x1D4790 - 0x26960 - (3 * (48 * 16) / 2) - 0x1000;
