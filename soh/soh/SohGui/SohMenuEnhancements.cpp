...
void SohMenu::AddMenuEnhancements() {
    ...
    AddWidget(path, "Get Item Manipulation (GIM)", WIDGET_CVAR_CHECKBOX)
        ...
        .Options(CheckboxOptions().Tooltip(
            "Restores Get Item Manipulation by emulating the out-of-bounds get-item table read that "
            "occurs on console when an item is received with a negative get-item ID."));
